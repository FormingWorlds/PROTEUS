...
- `input/` - TOML configuration files (see the rule below on what may live here)
...
**Campaign and production configurations are never committed to this repository.**
A configuration that defines a run whose output is a scientific result belongs
with that result: a campaign repository, the paper repository, or a Zenodo
deposit. This holds on every branch and in every pull request, whatever the file
is named and however convenient it is to keep it beside the code that reads it.

Such a configuration is the specification of one campaign, not part of the
framework. Committing it ties a mutable experiment to the code's release history.
It puts campaign-specific choices in front of every contributor reading the
repository. And it splits the provenance of a result across two repositories, so
neither is citable on its own.

The test is what the run is for, not what the file looks like. A grid definition
is not excluded by being a grid definition: `example.grid.toml` is one and belongs
here. A grid-shaped file is excluded only when it is the config behind a result
that will be reported. What `input/` ships is either a reference, a test fixture,
or a worked example that is regenerated from one of those:

- `all_options.toml` - the schema reference; a new module documents its options here.
- `dummy.toml` and `minimal.toml` - the fast configurations the test fixtures build on.
- `example.grid.toml` and `tutorials/` - worked examples for the tutorials and how-to guides.
- `chili/intercomp/` - the CHILI intercomparison set, regenerated by `tools/chili_generate.py`
  from the tutorial configs and held against drift by CI. It is a fixture tracking the
  protocol, not the frozen config behind any submission.
- `inference/` - worked examples for the inference workflow.

This list is not closed. A new reference, fixture, or worked example may be added
if it meets the same test; ask in the pull request when it is unclear.

Give a configuration kept outside this tree an absolute `ref_config`. Both
`proteus grid` and `proteus infer` join that value onto the PROTEUS directory,
which is located from the package's own file path rather than configured, so a
bare relative path resolves inside the repository. A relative path can still climb
out with `..`, but it then depends on where the package happens to sit; an
...
Under D1A (the chosen design), CALLIOPE / atmodeller chemistry is unchanged. Oxygen is treated as a buffered element at the chemistry step but a tracked element in PROTEUS-side mass accounting. The asymmetry that previously let `M_atm > M_planet` at high H budgets is closed by including O in M_ele, in the Zalmoxis dry-mass subtraction, in the proportional escape distribution, and in the desiccation gate. Escape includes O in the unfractionated partitioning so `sum(esc_rate_e) == esc_rate_total` to within rounding. The runtime invariant `M_atm <= M_planet` is enforced via `assert_mass_conservation` in the main loop, except when `outgas.vapourise = true`: rock vapourisation deliberately moves rock mass into `M_atm` without subtracting it from the interior, and rock-vapour elements dilute the escape outflow without being debited from a tracked reservoir. In that mode the main loop passes `require_atm_le_planet=False`, which disables that one half and replaces it with a warning raised whenever the excess over `M_planet` is larger than `M_vaps`, i.e. larger than vapourisation explains. The relaxation applies only while `M_vaps > 0`; with no vapour column present the invariant is enforced regardless of the keyword. The other half of the check (`M_vol_atm` equals the summed per-species atmospheric masses) stays enforced in both modes, and `atol_frac` is never loosened. Treat that non-conservation as intentional, not a bug to repair; see `docs/Explanations/model.md`, "Whole-planet mass is not conserved when vapourisation is enabled". An IC consistency check (`check_ic_oxygen_budget`, called once after the first outgas call) hard-fails on >50% divergence between user-supplied O_budget and CALLIOPE's equilibrium value.
