...
def gas_vmr_to_emr(gases: dict):
    ...
    M_planet = sum(list(M_e.values()))

    # Guard against zero or non-finite total mass to avoid division by zero
    if (not np.isfinite(M_planet)) or abs(M_planet) < 1e-30:
        log.warning(
            'gas_vmr_to_emr: total elemental mass M_planet is zero or invalid '
            'for provided gas VMRs; returning empty elemental mass ratios.'
        )
        return {}

    # Get mass mixing ratios of elements
    emr = {e: M_e[e] / M_planet for e in element_list if M_e[e] > 1e-20}
