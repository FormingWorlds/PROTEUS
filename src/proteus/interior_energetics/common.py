...
def compute_initial_entropy(
    ...
    try:
        ...
        # Find the P-S tables (generated by Zalmoxis)
        spider_eos_dir = None
        outdir = hf_row.get('_outdir', '') if hf_row else ''
        import glob
        candidates = [
            os.path.join(outdir, 'data', 'spider_eos'),
        ]
        for cand in candidates:
            if os.path.isdir(cand) and glob.glob(os.path.join(cand, 'temperature_melt.dat')):
                spider_eos_dir = cand
                break
        ...
        log.debug('P-S inversion failed in common.py: %s', e)
