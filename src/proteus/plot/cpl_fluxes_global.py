...
def plot_fluxes_global(
    hf_all: pd.DataFrame, output_dir: str, config: Config, t0: float = 100.0
):
    # Get values
    ...
    F_asf = (
        np.array(hf_crop['F_ins'])
        * config.orbit.s0_factor
        * (1.0 - config.atmos_clim.albedo_pl)
    ...
    ax.plot(
        time, F_asf, lw=lw, alpha=al, color=get_colour('ASF'), label='ASF', linestyle='dotted'
    )
