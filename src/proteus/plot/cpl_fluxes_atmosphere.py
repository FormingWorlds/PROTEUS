...
def plot_fluxes_atmosphere(output_dir: str, plot_format='pdf'):
    ...
    # Axis limits
    max_fl = max(100.0, np.amax(np.abs(atm_fl_T)), np.amax(atm_fl_U), np.amax(atm_fl_D)) * 1.1
    ...
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1), handletextpad=0.4, labelspacing=0.3)
