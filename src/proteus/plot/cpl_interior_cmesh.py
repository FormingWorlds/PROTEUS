...
def plot_interior_cmesh(
    ...
    cb.set_ticks(
        [float(round(v, -2)) for v in np.linspace(np.amin(arr_z1), np.amax(arr_z1), numticks)]
    )
    ...
    if use_contour:
        cf = ax2.contourf(
            ...
            cmap=cmap,
            norm=norm,
            levels=np.linspace(0.0, 1.0, cblevels),
        )
    else:
    ...
    if use_contour:
        cf = ax4.contourf(
            times, arr_yb, arr_z4.T, cmap=cmap, norm=norm, levels=cblevels, extend='min'
        )
    else:
        cf = ax4.pcolormesh(
            times, arr_yb, arr_z4.T, cmap=cmap, norm=norm, rasterized=True, extend='min'
        )
