...
def run_obliqua(hf_row:dict, dirs:dict, interior_o:Interior_t, config:Config) -> Tuple[np.array, np.array]:
    ...
    M_star = _jlsca(hf_row["M_star"])

    # Convert planet-satellite orbital eccentricity, semi-major axis, and mass
    #ecc_sat = _jlsca(hf_row["eccentricity_sat"])
    #sma_sat = _jlsca(hf_row["semimajorax_sat"])
    #M_sat   = _jlsca(hf_row["M_sat"])
    ...
    # Return the frequency dependent Imk2 spectrum
    return np.array(σ_range), np.array(Imk2)
