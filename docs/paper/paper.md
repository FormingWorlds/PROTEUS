...
It is critically important for the exploration of the exoplanet census and refined understanding of the deep history of the terrestrial planets that a variety of independent models are developed, optimally in an open source fashion, so that individual approaches can be compared against one another, and the community can learn from each other and thus produce better and more robust science. A detailed comparison with these codes would go way beyond the limits of this article. Hence, we here limit our discussion to the traits that we believe are the unique capabilities and implementation aspects of PROTEUS. These are:
  ...
  - The dynamic resolution of interior and atmospheric energy transfers regimes: radiative and convective layers in the atmosphere; conduction and convection in multiple dynamics regimes are resolved.
  ...
  - Interconnected atmospheric escape that couples to the planetary interior; i.e., the escaping reservoir is linked and/or disconnected from the 'hidden' reservoir in the deep interior, depending on evolutionary state.
  - Time-resolved changes in the stellar spectrum and energy flux for a wide array of stellar types.
  - Inclusion of equilibrium and disequilibrium chemistry in the atmosphere.
  - The inclusion of realistic, measure surface reflection properties for solid and molten surface conditions.
  ...
  - Automated testing of individual modules and the PROTEUS framework as a whole using GitHub CLI.
