...
With `mantle_eos = "PALEOS:H2O"` or `"PALEOS:iron"` and SPIDER or Aragog energetics, the structure uses the density of that material while the energetics use the MgSiO$_3$ melting curves and pressure-entropy tables; the run logs one warning about this at the start of each run segment, so a resumed run logs it again.
