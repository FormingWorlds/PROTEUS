...
| `eddy_diffusivity_thermal` | float | `1.0` | Thermal eddy diffusivity control. A positive value multiplies the internally-computed eddy diffusivity; a negative value pins it to \|value\| in m^2/s before ``kappah_floor`` is applied, so in a convectively unstable cell the pinned value can still be lifted to the floor. A value of -1.0 is not a sentinel here; it pins the diffusivity to 1 m^2/s like any other negative value. Matches SPIDER -eddy_diffusivity_thermal (1.0 default). |
| `eddy_diffusivity_chemical` | float | `1.0` | Chemical eddy diffusivity control. A positive value multiplies the internally-computed eddy diffusivity; a negative value pins it to \|value\| in m^2/s; this field has no floor. A value of -1.0 is not a sentinel here; it pins the diffusivity to 1 m^2/s like any other negative value. Matches SPIDER -eddy_diffusivity_chemical (1.0 default). |
