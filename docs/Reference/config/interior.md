...
With `mantle_eos = "PALEOS:H2O"` or `"PALEOS:iron"`, the structure uses the density of that material while SPIDER or Aragog energetics use the MgSiO$_3$ melting curves and pressure-entropy tables, and `temperature_mode = "liquidus_super"` solves the initial adiabat on the MgSiO$_3$ two-phase tables; the run logs one warning that names the cases that apply at the start of each run segment, so a resumed run logs it again.
