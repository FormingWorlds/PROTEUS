...
A resumed SPIDER run that keeps its pressure-entropy tables needs only the tables of its layers; an Aragog resume also needs the two-phase pair, which it reads when the mesh changes.
