...
A run with a PALEOS EOS requires the tables the chosen EOS reads: the table of each PALEOS layer (the MgSiO$_3$ unified table only when a layer is `PALEOS:MgSiO3`) and, for a mantle with a PALEOS component, the MgSiO$_3$ two-phase solid and liquid pair (the high-resolution pair for `PALEOS-2phase:MgSiO3-highres`, the PALEOS-API cache for the PALEOS-API family).
...
A resumed run that keeps its pressure-entropy tables needs only the tables of its layers.
A mixture mantle (a `+` in `mantle_eos`) takes its melting curves, in the structure and in the energetics, and its SPIDER or Aragog tables from its MgSiO$_3$ component: with `PALEOS:MgSiO3` it gets exactly the curves and tables of a `PALEOS:MgSiO3` mantle, and with `WolfBower2018:MgSiO3` the Wolf and Bower (2018) tables and the `melting_dir` curves. The other components enter only the structure density.
A PALEOS mantle with no MgSiO$_3$ component (for example `mantle_eos = "PALEOS:H2O"`, `"PALEOS:iron"`, or a mixture of the two) uses the density of its own material in the structure, while the PALEOS MgSiO$_3$ melting curves and the two-phase MgSiO$_3$ tables of its PALEOS family stand in for the energetics; `temperature_mode = "liquidus_super"` solves the initial adiabat on the MgSiO$_3$ two-phase tables.
