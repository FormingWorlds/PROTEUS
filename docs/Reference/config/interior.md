...
A resumed SPIDER run that keeps its pressure-entropy tables needs only the tables of its layers, and with `temperature_mode = "liquidus_super"` also the two-phase pair its anchor reads; an Aragog resume always needs the two-phase pair, which it reads when the mesh changes.
