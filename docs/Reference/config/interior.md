...
A PALEOS mantle with no MgSiO$_3$ component (for example `mantle_eos = "PALEOS:H2O"`, `"PALEOS:iron"`, or a mixture of the two) uses the density of its own material in the structure, while MgSiO$_3$ tables stand in elsewhere: SPIDER or Aragog energetics use the MgSiO$_3$ melting curves and pressure-entropy tables of PALEOS for a single PALEOS component, and the Wolf and Bower (2018) tables with the `melting_dir` curves for a mixture; `temperature_mode = "liquidus_super"` solves the initial adiabat on the MgSiO$_3$ two-phase tables.
