...
A run with a PALEOS EOS requires the full PALEOS table set: the table of each PALEOS layer and, for a mantle with a PALEOS component, the MgSiO$_3$ two-phase solid and liquid pair that the energetics read (the high-resolution pair for `PALEOS-2phase:MgSiO3-highres`, the PALEOS-API cache for the PALEOS-API family) together with the MgSiO$_3$ unified table of the same family.
...
With `mantle_eos = "PALEOS:H2O"` or `"PALEOS:iron"`, the structure uses the density of that material while the energetics use the MgSiO$_3$ melting curves and pressure-entropy tables; the run logs one warning about this at the start of each run segment, so a resumed run logs it again.
