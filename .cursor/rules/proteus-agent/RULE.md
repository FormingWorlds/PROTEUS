---
description: "PROTEUS Agent Onboarding Guide - Comprehensive instructions for working on the PROTEUS ecosystem, including installation, testing, code quality standards, and project structure"
alwaysApply: true
---
...
**Verify test discovery** (before writing tests):
...
- `@pytest.mark.slow` - Full physics validation (hours; mark slow tests with full simulation loops)

**Rules**:

- **Never** use `==` for floats. Use `pytest.approx(val, rel=1e-5)` or `np.testing.assert_allclose`
- **Always** mock external calls (SOCRATES, AGNI, file I/O, network) in unit tests using `unittest.mock`
- **Always** use physically valid inputs (T > 0K, P > 0) unless testing error handling
- **Always** read `tests/conftest.py` (or module-specific `conftest.py`) before writing tests to utilize existing fixtures
- **Always** add detailed docstrings explaining the physical scenario being tested
- **Prefer** `@pytest.mark.parametrize` over writing multiple similar test functions
- **Include** detailed comments explaining *why* a specific input range was chosen (e.g., "Temperature set to 300K to represent habitable zone conditions")
...
- **Physics Checks**: Include detailed comments explaining *why* a specific input range was chosen
- **Documentation**:
  - Add detailed docstrings to each test explaining the physical scenario being tested
  - In the header of the test file, include a brief overview of what is being tested and any important context
  - Include links to all documentation about testing standards: `docs/test_infrastructure.md`, `docs/test_categorization.md`, and `docs/test_building.md`
- **Instructions**: See `docs/test_building.md` for best practices on building robust tests and `docs/test_building_strategy.md` for current test building strategy
...
**Target**: Aim for >90% per module. Use `pytest --cov=src --cov-report=html` to identify gaps.

## Code Quality & Style

**Linting**: Follow `ruff` standards.

**Style** (enforced by ruff):

- Line length < 92 chars (preferred; config allows up to 96)
...
**Randomness**: Explicitly set seeds (e.g., `np.random.seed(42)`) in tests to ensure reproducibility.

**Files**: Do not generate tests that produce large output files (unless explicitly instructed); use `tempfile` or mocks instead.
...
- **Testing**:
  - `docs/test_infrastructure.md` - Full testing infrastructure details
  - `docs/test_building.md` - Best practices on building robust tests
  - `docs/test_building_strategy.md` - Current test building strategy
  - `docs/test_categorization.md` - Test categorization guidelines
