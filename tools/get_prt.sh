...
sed -i.bak '0,/^    except ImportError:$/{s/^    except ImportError:$/    except Exception:/;}' petitRADTRANS/__file_conversion.py
sed -i.bak '0,/^    except ImportError:$/{s/^    except ImportError:$/    except Exception:/;}' petitRADTRANS/opacities/opacities.py
...
sed -i.bak "s|^prt_input_data_path = .*|prt_input_data_path = ${prt_input_data_path}|" ~/.petitradtrans/petitradtrans_config_file.ini
sed -i.bak '/^\[Paths\]/i\
