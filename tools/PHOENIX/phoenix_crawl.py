...
def convert_with_medres(medres_module, fits_path: Path, out_dir: Path):
    ...
    if 'HiRes' in out_name:
        out_name = out_name.replace(
            'HiRes', 'MedRes'
        )  # For some reason, PHOENIX names R100000 MedRes files with HiRes
...
def download_and_process_zip(
    zip_url: str, work_dir: Path, out_dir: Path, medres_module, overwrite=False
):
...
def main():
    ap = argparse.ArgumentParser(
        description='Fetch PHOENIX MedRes R=10000 spectra and convert with openmedres.py.'
    )
    ap.add_argument('--base-url', default=BASE_URL, help='Base directory for R=10000 ZIPs.')
    ap.add_argument(
        '--outdir', default='phoenix_out', help='Destination folder for .txt files.'
    )
    ap.add_argument(
        '--workdir', default='tmp', help='Temporary folder for downloads/extraction.'
    )
    ap.add_argument(
        '--overwrite',
        action='store_true',
    ...
    ap.add_argument(
        '--medres-module',
        default='openmedres',
        help='Converter module name (default medres.py).',
    )
    args = ap.parse_args()
    ...
    for u in zip_urls:
        try:
            download_and_process_zip(
                u, work_dir, out_dir, medres_module, overwrite=args.overwrite
            )
        except Exception as e:
