...
if __name__=='__main__':
    ...
    config = "planets/nogit_fiducial_sub_Neptune_planetmass.toml"
    folder = "SNS_planetmass_several_amf_study"
    
    use_custom_grid = False
    
    custom_grid_name = "nogit_csv_inputs/SNS_metallicity_grid_carbon.csv"
    
    with open(custom_grid_name, "r") as f:
        reader = csv.DictReader(f)
        custom_grid_flat = [
            {
                "delivery.elements.H_ppmw": float(row["delivery.elements.H_ppmw"]), 
                "delivery.elements.C_ppmw": float(row["delivery.elements.C_ppmw"]),
                "delivery.elements.N_ppmw": float(row["delivery.elements.N_ppmw"]),
                "outgas.fO2_shift_IW": float(row["outgas.fO2_shift_IW"])
            }
            for row in reader
        ]
    ...
    # Execution limits
    max_jobs = 40           # maximum number of concurrent tasks
    ...
    if not use_custom_grid:
        ...
        pg.set_dimension_direct("hbudget", [709,7090,35461])

        pg.add_dimension("mass", "struct.mass_tot")
        pg.set_dimension_direct("mass", np.linspace(2,8,13))
