...
temperature-based T(P) curves. The T(P) curves are read from the local folder
FWL_DATA/interior_lookup_tables/Melting_curves/<melting_dir>/ when it holds
them, otherwise from the fetched melting-curve dataset of that name. The S(P)
files are written into the local folder, which SPIDER reads:
...
------------
    ...
    - T(P) melting curves: solidus_P-T.dat and liquidus_P-T.dat in the local
      melting_dir folder, or a fetched curve (Monteux+600, Monteux-600,
      Wolf_Bower+2018; run `proteus get interiordata` first)
"""
...
def find_tp_curves(melting_dir: str) -> tuple[Path, Path]:
    """Return the T(P) solidus and liquidus files for one melting curve.

    Parameters
    ----------
    melting_dir : str
        Melting curve name, as in ``interior_struct.melting_dir``.

    Returns
    -------
    tuple of Path
        Solidus and liquidus files: the local folder's ``*_P-T.dat`` or
        ``solidus.dat``/``liquidus.dat`` pair, else the fetched dataset's files.
    """
    local = MELTING_CURVES_BASE / melting_dir
    for sol, liq in (('solidus_P-T.dat', 'liquidus_P-T.dat'), ('solidus.dat', 'liquidus.dat')):
        if (local / sol).is_file() and (local / liq).is_file():
            return local / sol, local / liq
    from proteus.utils.data import resolve_melting_curve_files

    return resolve_melting_curve_files(melting_dir, data_root=FWL_DATA)
...
def main():
    ...
    # T(P) input: local folder first, then the fetched dataset. The S(P) output
    # always goes to the local folder, which SPIDER reads.
    mc_folder = MELTING_CURVES_BASE / args.melting_dir
    solidus_tp, liquidus_tp = find_tp_curves(args.melting_dir)
    ...
    print(f'\nLoading T(P) curves from {solidus_tp.parent}/')
