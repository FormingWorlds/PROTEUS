...
temperature-based T(P) curves. The T(P) curves are the ones a run reads: the
local folder FWL_DATA/interior_lookup_tables/Melting_curves/<melting_dir>/ when
it holds solidus_P-T.dat and liquidus_P-T.dat, otherwise the fetched
melting-curve dataset of that name. The S(P) files are written into the local
folder, where SPIDER looks for P-S curves when the run supplies none of its own:
...
------------
    ...
    - PROTEUS importable, for the melting-curve resolver
"""
...
def find_tp_curves(melting_dir: str) -> tuple[Path, Path]:
    ...
    -------
    tuple of Path
        Solidus and liquidus files, resolved as a PROTEUS run resolves them.
    """
