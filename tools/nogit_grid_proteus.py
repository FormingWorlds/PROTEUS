...
class Grid():
    ...
    def __init__(self, name:str, base_config_path:str, custom_grid_flat:list=None, symlink_dir:str="_UNSET"):
        ...
        if self.symlink_dir in ["_UNSET", None]:
            ...
            if os.path.exists(self.symlink_dir):
                ...
                if ".git" in subfolders:
                    raise Exception("Not emptyting directory - it contains a Git repository!")
                time.sleep(4.0)
...
if __name__=='__main__':
    ...
    parser.add_argument("usegrid", type=str)
    parser.add_argument("gridname", type=str)
    parser.add_argument("foldername", type=str)
    parser.add_argument("threads", type=int)
    args = parser.parse_args()
    
    config = "planets/nogit_fiducial_sub_Neptune.toml"
    folder = args.foldername
    
    use_custom_grid = args.usegrid
    
    custom_grid_name = args.gridname
    custom_grid_flat = None
            ...
            custom_grid_flat = [
                {
                    ...
                    "orbit.instellationflux": float(row["orbit.instellationflux"])
                }
                for row in reader
            ]
    ...
    max_days = 7            # maximum number of days to run
    ...
    if use_custom_grid=='False': 
        ...
        pg.add_dimension("teff", "star.dummy.Teff")
        pg.set_dimension_linspace("teff", 4900, 6300, 8)

        pg.add_dimension("finst", "orbit.instellationflux")
        pg.set_dimension_linspace("finst", 11, 12, 10)

        pg.add_dimension("hbudget", "delivery.elements.H_ppmw")
        pg.set_dimension_direct("hbudget", [2128])
