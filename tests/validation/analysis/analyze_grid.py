...
def plot_thermal_evolution_comparison():
    ...
    fig, axes = plt.subplots(len(masses), len(cmfs), figsize=(20, 22),
                              sharex=False, sharey=False)
    fig.suptitle("Block A: Thermal Evolution — AW (blue) vs ZAL (red)",
    ...
    for i, mass in enumerate(masses):
        for j, cmf in enumerate(cmfs):
            ...
            for name, color, label in [(aw_name, AW_COLOR, "AW"),
                                        (zal_name, ZAL_COLOR, "ZAL")]:
                if name in cases:
                    c = cases[name]
            ...
            for name, color in [(aw_name, AW_COLOR), (zal_name, ZAL_COLOR)]:
                if name in cases:
                    ...
                    if c["steps"] < 15 and c["t_final"] < 500:
                        ax.text(0.5, 0.5, f"{'AW' if 'AW' in name else 'ZAL'}\nSTUCK",
...
def plot_melt_fraction_evolution():
    ...
    fig.suptitle("Block A: Melt Fraction Evolution — AW (blue) vs ZAL (red)",
                 fontsize=16, fontweight="bold", y=0.98)
...
def plot_solidification_times():
        ...
        for cmf in cmfs:
            ...
            if aw_name in cases and zal_name in cases:
                ...
                if aw["solidified"] and zal["solidified"]:
                    ax.scatter(aw["t_final"], zal["t_final"],
    ...
    ax.fill_between(lims, [l*0.8 for l in lims], [l*1.2 for l in lims],
                    alpha=0.1, color="gray", label="$\\pm$20%")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlim(lims)
    ax.set_ylim(lims)
    ...
    mass_handles = [Line2D([0], [0], marker=MASS_MARKERS[m], color="gray",
                           markersize=8, linestyle="none",
                           label=f"{m} M$_\\oplus$") for m in masses]
    cmf_handles = [Line2D([0], [0], marker="o", color=CMF_COLORS[c],
                          markersize=8, linestyle="none",
                          label=f"CMF={c}") for c in cmfs]
    ax.legend(handles=mass_handles + cmf_handles, loc="upper left",
              fontsize=7, ncol=2)
    ...
    for mass in masses:
        for cmf in cmfs:
            ...
            if aw_name in cases and zal_name in cases:
                ...
                zal = cases[zal_name]
                if aw["solidified"] and zal["solidified"]:
                    ax.scatter(aw["T_magma_final"], zal["T_magma_final"],
                              color=CMF_COLORS[cmf],
                              marker=MASS_MARKERS[mass],
                              s=100, edgecolors="black", linewidths=0.5,
                              zorder=5)
    ...
    ax.fill_between(lims, [l*0.95 for l in lims], [l*1.05 for l in lims],
                    alpha=0.1, color="gray", label="$\\pm$5%")
    ax.set_xlim(lims)
    ax.set_ylim(lims)
...
def plot_stability_map():
        ...
        for mass in masses_all:
            for cmf in cmfs_all:
                ...
                if found is None:
                    ...
                    ax.scatter(cmf, mass, marker="x", color="black", s=80,
                              linewidths=2, zorder=5)
                elif found["solidified"]:
                    ax.scatter(cmf, mass, marker="o", color="green", s=120,
                              edgecolors="black", linewidths=0.5, zorder=5)
                elif found["steps"] < 15 and found["t_final"] < 500:
                    ax.scatter(cmf, mass, marker="s", color="red", s=120,
                              edgecolors="black", linewidths=0.5, zorder=5)
                elif found["Phi_final"] > 0.01 and found["t_final"] > 5e5:
                    ax.scatter(cmf, mass, marker="D", color="orange", s=120,
                              edgecolors="black", linewidths=0.5, zorder=5)
                else:
                    ax.scatter(cmf, mass, marker="^", color="blue", s=120,
                              edgecolors="black", linewidths=0.5, zorder=5)
    ...
    legend_elements = [
        Line2D([0], [0], marker="o", color="green", markersize=10,
               linestyle="none", label="Solidified ($\\Phi < 0.01$)"),
        Line2D([0], [0], marker="D", color="orange", markersize=10,
               linestyle="none", label="Persistent melt ($t > 0.5$ Myr)"),
        Line2D([0], [0], marker="^", color="blue", markersize=10,
               linestyle="none", label="Still running"),
        Line2D([0], [0], marker="s", color="red", markersize=10,
               linestyle="none", label="Stuck (EOS stiffness)"),
        Line2D([0], [0], marker="x", color="black", markersize=10,
               linestyle="none", label="Failed / not in grid"),
    ]
...
def plot_structural_comparison():
    ...
    # Collect data
    data = {"mass": [], "cmf": [], "R_aw": [], "R_zal": [],
            "M_aw": [], "M_zal": [], "g_aw": [], "g_zal": [],
            "Mc_aw": [], "Mc_zal": []}
    ...
    for i in range(len(data["mass"])):
        ax.scatter(data["R_aw"][i], data["R_zal"][i],
    ...
    ax = axes[0, 1]
    for i in range(len(data["mass"])):
        ax.scatter(data["M_aw"][i], data["M_zal"][i],
                  color=CMF_COLORS[data["cmf"][i]],
                  marker=MASS_MARKERS[data["mass"][i]],
                  s=100, edgecolors="black", linewidths=0.5)
    lims = [0, 6]
    ...
    ax = axes[1, 0]
    for i in range(len(data["mass"])):
        ax.scatter(data["g_aw"][i], data["g_zal"][i],
                  color=CMF_COLORS[data["cmf"][i]],
                  marker=MASS_MARKERS[data["mass"][i]],
                  s=100, edgecolors="black", linewidths=0.5)
    lims = [0, 30]
    ...
    ax = axes[1, 1]
    rel_R = [(data["R_zal"][i] - data["R_aw"][i]) / data["R_aw"][i] * 100
             for i in range(len(data["mass"]))]
    rel_M = [(data["M_zal"][i] - data["M_aw"][i]) / data["M_aw"][i] * 100
             for i in range(len(data["mass"]))]
    rel_g = [(data["g_zal"][i] - data["g_aw"][i]) / data["g_aw"][i] * 100
             for i in range(len(data["mass"]))]
        ...
        for name, color, ls in [(aw_name, AW_COLOR, "-"), (zal_name, ZAL_COLOR, "-")]:
            if name in cases:
                ...
                if t:
                    ...
                    ax.plot(t, fint, color=color, linewidth=1.5, linestyle="-",
                            label=f"{mode} $F_\\mathrm{{int}}$")
                    ax.plot(t, fatm, color=color, linewidth=1.0, linestyle="--",
                            label=f"{mode} $F_\\mathrm{{atm}}$", alpha=0.7)
...
def plot_resolution_convergence():
    ...
    table = ax.table(cellText=table_data,
                     colLabels=["Resolution", "Steps", "t_final [yr]",
                               "T_magma [K]", "Phi_final"],
    ...
    for ci, cmf in enumerate(cmfs):
        ...
        for S in entropies:
            ...
            if name in cases:
                ...
                t = [r["Time"] for r in c["rows"]]
                T = [r["T_magma"] for r in c["rows"]]
                ax.plot(t, T, color=ent_colors[S], linewidth=1.5,
                       label=f"$S_0$={S}")
        ax.set_xlabel("Time [yr]")
        ax.set_ylabel("$T_\\mathrm{magma}$ [K]")
        ...
        for S in entropies:
            ...
            if name in cases:
                ...
                t = [r["Time"] for r in c["rows"]]
                phi = [r["Phi_global"] for r in c["rows"]]
                ax.plot(t, phi, color=ent_colors[S], linewidth=1.5,
                       label=f"$S_0$={S}")
        ax.axhline(0.01, color="gray", linestyle="--", linewidth=0.7, alpha=0.5)
        ax.set_xlabel("Time [yr]")
        ax.set_ylabel("$\\Phi_\\mathrm{global}$")
...
def plot_high_mass_frontier():
    ...
    fig.suptitle("High-Mass Frontier: AW (top, 7–10 M$_\\oplus$) vs ZAL (bottom, 7 M$_\\oplus$)",
    ...
    for j, cmf in enumerate(cmfs):
        ...
        for mass, ls in [(7.0, "-"), (10.0, "--")]:
            ...
            if name in cases:
                ...
                T = [r["T_magma"] for r in c["rows"]]
                if t and max(t) > 0:
                    ax.plot(t, T, color=AW_COLOR, linewidth=1.5, linestyle=ls,
                           label=f"AW {mass} M$_\\oplus$")
                elif t:
                    ax.text(0.5, 0.5, f"AW {mass}\nSTUCK",
                           transform=ax.transAxes, ha="center", va="center",
    ...
    for j, cmf in enumerate(cmfs):
        ...
        if name in cases:
            ...
            phi = [r["Phi_global"] for r in c["rows"]]
            if t and max(t) > 0:
                ax.plot(t, T, color=ZAL_COLOR, linewidth=1.5,
                       label="ZAL 7 M$_\\oplus$")
                ax2 = ax.twinx()
                ax2.plot(t, phi, color=ZAL_COLOR, linewidth=1.0,
                        linestyle="--", alpha=0.5)
...
def plot_phase2_feedback():
    ...
    for idx, mass in enumerate(masses):
        ...
        for name, color, label in [(p1_name, "#1b9e77", "Phase 1 (static mesh)"),
...
def plot_persistent_melt():
    ...
    for name, c in persist_cases[:8]:
        ...
        if t:
            ax.plot(t, T, linewidth=1.2,
                   label=f"{c['mass']}M {c['mode']} CMF{c['cmf']}")
    ax.set_xscale("log")
    ax.set_xlabel("Time [yr]")
    ax.set_ylabel("$T_\\mathrm{magma}$ [K]")
    ...
    for name, c in persist_cases[:10]:
        table_data.append([
            f"{c['mass']}", f"{c['cmf']}", c["mode"],
            f"{c['steps']}", f"{c['t_final']/1e6:.2f}",
            f"{c['T_magma_final']:.0f}", f"{c['Phi_final']:.3f}",
        ])
    if table_data:
        table = ax.table(cellText=table_data,
                         colLabels=["Mass", "CMF", "Mode", "Steps",
                                   "t [Myr]", "T_mag [K]", "Phi"],
                         loc="center", cellLoc="center")
        table.auto_set_font_size(False)
...
def plot_summary():
    ...
    for cmf in cmfs:
        ...
        if aw_m:
            ax.plot(aw_m, aw_t, "o--", color=CMF_COLORS[cmf], markersize=8,
                   label=f"AW CMF={cmf}")
        if zal_m:
            ax.plot(zal_m, zal_t, "s-", color=CMF_COLORS[cmf], markersize=8,
                   markerfacecolor="white", markeredgecolor=CMF_COLORS[cmf],
                   markeredgewidth=2, label=f"ZAL CMF={cmf}")
    ...
    for cmf in cmfs:
        ...
        if zal_m:
            ax.plot(zal_m, zal_s, "s-", color=CMF_COLORS[cmf], markersize=8,
                   markerfacecolor="white", markeredgecolor=CMF_COLORS[cmf],
                   markeredgewidth=2)
    ax.set_xlabel("Planet Mass [M$_\\oplus$]")
