#!/usr/bin/env python
"""Generate Habrok validation grid v3 for Zalmoxis-SPIDER coupling.

Post-adiabat-fix validation focused on 1-3 M_earth. Tests the native
dT/dP table adiabat, AW vs Zalmoxis mesh comparison, Phase 2 structural
feedback, and resolution convergence.

Test blocks
-----------
A : AW baseline                    3 masses x 3 CMFs               =  9
B : ZAL adiabatic Phase 1         3 masses x 3 CMFs               =  9
C : ZAL linear T control          3 masses x 2 CMFs               =  6
D : Phase 2 structural feedback   3 masses x 3 CMFs               =  9
E : Resolution convergence        3 num_levels values              =  3
                                                             Total = 36

Key comparisons:
  A vs B : Does Zalmoxis mesh change solidification physics?
  B vs C : Does the native dT/dP adiabat improve initial conditions?
  B vs D : Does structural re-computation affect evolution?
  E      : Is num_levels=60 sufficient for 3 M_earth?

Usage
-----
    # Generate configs (on Habrok login node)
    python generate_habrok_grid.py --outdir /scratch/$USER/habrok_validation_v3

    # Submit to SLURM
    sbatch /scratch/$USER/habrok_validation_v3/slurm_dispatch.sh
...
2. SPIDER compiled from tl/external-mesh branch
3. Zalmoxis on tl/adiabatic-temperature branch (native dT/dP adiabat fix)
...
BLOCK_LABELS = {
    'A': 'AW baseline',
    'B': 'ZAL adiabatic Phase 1',
    'C': 'ZAL linear T control',
...
def build_cases() -> list[Case]:
    """Build the v3 validation matrix (1-3 M_earth, post-adiabat-fix).
    ...
    # Block A: AW baseline (9 cases)
    # Adams-Williamson internal mesh; no Zalmoxis involvement.
    # Establishes reference solidification curves for all mass-CMF combos.
    for mass in masses:
        for cmf in cmfs:
            cases.append(Case('A', mass, cmf, 'self'))

    # Block B: ZAL adiabatic Phase 1 (9 cases)
    # Zalmoxis external mesh with native dT/dP adiabat (auto-switched).
    # Primary block testing the adiabat fix — v2 crashed for CMF<=0.325.
    for mass in masses:
        for cmf in cmfs:
            cases.append(Case('B', mass, cmf, 'zalmoxis'))

    # Block C: ZAL linear T control (6 cases)
    # Force temperature_mode="linear" to isolate the effect of the
    # adiabat initial condition. CMF=0.5 omitted (stable in v2).
    for mass in masses:
        for cmf in [0.10, 0.325]:
            cases.append(Case('C', mass, cmf, 'zalmoxis', temperature_mode='linear'))

    # Block D: Phase 2 structural feedback (9 cases)
    # Zalmoxis re-computes structure every 100 yr during SPIDER evolution.
    # Compared pair-wise against Block B (same config minus Phase 2).
    for mass in masses:
        for cmf in cmfs:
            cases.append(Case('D', mass, cmf, 'zalmoxis', update_interval=100.0))

    # Block E: Resolution convergence (3 cases)
    # Test mesh sensitivity for the hardest mass-CMF combination.
    # Reference n=60 is in Block B (B_M3.0_CMF0.325_ZAL).
    for nl in [40, 90, 120]:
...
def apply_overrides(config: dict, case: Case, case_outdir: str) -> dict:
    ...
    # Override temperature mode if specified (e.g. Block C forces linear)
...
def generate_slurm_script(outdir: Path, n_cases: int) -> Path:
    ...
    script = f"""\
#!/bin/bash
#SBATCH -J spider-val-v3
#SBATCH --partition=regular
#SBATCH --time=0-12:00:00
#SBATCH --mem-per-cpu=3G
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --array=0-{n_cases - 1}%{min(n_cases, 36)}
...
echo "========================================"
echo "SPIDER validation grid v3 (1-3 M_earth)"
...
def main():
    """Generate Habrok validation grid v3."""
    parser = argparse.ArgumentParser(
        description='Generate Habrok validation grid v3 for Zalmoxis-SPIDER coupling',
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help='Root output directory (e.g. /scratch/$USER/habrok_validation_v3)',
    )
    parser.add_argument(
        '--base-config',
        type=Path,
    ...
    print(f'Habrok validation grid v3: {len(cases)} cases\n')
