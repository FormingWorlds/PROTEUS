...
- `FWL_DATA_DIR` is read from `FWL_DATA` at import in `proteus.utils.data`, `proteus.interior_energetics.aragog`, `proteus.interior_energetics.spider` (with paths derived from it) and `proteus.interior_struct.zalmoxis`; patch the constant the code under test reads with `monkeypatch.setattr(..., raising=False)` as well as `setenv`.
