...
class InitialAbun(object):
    ...
    def ini_y(self, data_var, data_atm): 
        ...
        if vulcan_cfg.ini_mix == 'EQ':
        ...
        elif vulcan_cfg.ini_mix == 'spider':
            
            spider_abun = np.genfromtxt(vulcan_cfg.spider_file, names=True, dtype=None)
            ...
            with open('fastchem_vulcan/input/element_abundances_lodders.dat' ,'r') as f:
                ...
                # using costomized elemental abundances provided by spider
                print ("Initializing with the elemental abundances by SPIDER:")
                print ("{:4}".format('H') + str('1.'))
                for line in f.readlines():   
                    li = line.split()
                    if li[0] in ele_list:
                        sp = li[0].strip()
                        sp_abun = spider_abun[sp][-1]
                        fc_abun = 12. + np.log10(sp_abun)
                        line = sp + '\t' + "{0:.4f}".format(fc_abun) + '\n'
                        print ("{:4}".format(sp) + "{0:.4E}".format(sp_abun))
                    new_str += line
...
class Atm(object):
    ...
    def load_TPK(self, data_atm):
        ...
        elif self.type == 'socrates': # const Kzz
            
            atm_table = np.genfromtxt(vulcan_cfg.atm_file, names=['T', 'Pbar'], dtype=None, skip_header=0)
            p_file, T_file = atm_table['Pbar']*1e6, atm_table['T']
