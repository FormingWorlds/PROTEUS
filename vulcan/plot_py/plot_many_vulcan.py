...
vul_data2 = '../output/HD189_C2H2.vul'
...
for color_index,sp in enumerate(plot_spec):
    ...
    if sp in data2['variable']['species']:
        plt.plot(data2['variable']['ymix'][:,vulcan_spec2.index(sp)], data2['atm']['pco']/1.e6, color=tableau20[color_index], ls='--',lw=1.2, alpha=0.9)
    ...
    #     plt.plot(data3['variable']['ymix'][:,vulcan_spec2.index(sp)], data3['atm']['pco']/1.e6, color=colors[color_index], ls='-.',lw=1.2, alpha=0.9)

      
plt.gca().set_xscale('log')       
plt.gca().set_yscale('log') 
plt.gca().invert_yaxis() 
plt.xlim((1.E-16, 0.999))
plt.ylim((1.E3,1.E-8))
