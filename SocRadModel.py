...
def radCompSoc(atm, stellar_toa_heating):
    ...
    # Define temperature and pressure profiles
    ...
    # q_mr_list   = atm.mixing_ratios[0] # h2o
    h2o_mr_list = atm.mixing_ratios[0]
    ...
    nctools.ncout3d('profile.h2o',0,0,pres_list,h2o_mr_list,'h2o',longname="h2o",units='PPMV')
    ...
    # Define SOCRATES spectral file to use
    spectral_file_name = "gen_h2+n2+h2o+co2+co"

    seq4     = ("Cl_run_cdf -B", basename,"-s spectral-files/"+spectral_file_name+"/sp_spider -R 1 300 -ch 300 -S -g 2 -C 5 -u")
    seq5     = ("fmove", basename,"currentsw")
    seq6     = ("Cl_run_cdf -B", basename,"-s spectral-files/"+spectral_file_name+"/sp_spider -R 1 300 -ch 300 -I -g 2 -C 5 -u")
