...
def B(nu,T):
    ...
    for i in range(len(nu)):
        u = min(h*nu[i]/(k*T),500.) #To prevent overflow
        Bi=(2.*h*nu[i]**3/c**2)/(np.exp(u)-1.) # np.exp can take arrays as arguments, math.exp cannot.
        Bsum+=Bi
    return Bsum # Sum over all frequencies in freq(lam), should be close to sigma*T**4.
"""
def dB(nu,T):
    u = min(h*nu/(k*T),500.) #To prevent overflow
    dB=(2*h**2*nu**4/(c**2*k*T**2))*(math.exp(u)/(math.exp(u)-1)**2) # np.exp can take arrays as arguments, math.exp cannot.
    return dB
