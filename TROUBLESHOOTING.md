...
With the new Apple Silicon hardware, the option `-march=native` sometimes causes issues. In order to avoid this, you need to make sure to use the GNU version of `gcc`, not the Apple one. The Apple one located at `/usr/bin/gcc` is actually a wrapped around `clang`. I have found that using the Homebrew version located at `/opt/homebrew/bin/gcc-12` works well. To fix this error, edit the file `make.globaloptions` in the FastChem directory to use `gcc-12` instead of `gcc`.
